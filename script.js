...
const i18n = {
  en: {
   ...
   offlineNotice: "📴 Offline — changes will sync when reconnected"
  },
  gu: {
   ...
   offlineNotice: "📴 ઓફલાઇન — ઇન્ટરનેટ આવશે ત્યારે ડેટા સિંક થશે"
  }
 };
...
if (isIOS && !isStandalone && !alreadyInstalled) {
 ...
 // Watch for a new version. Also handles the case where a new version was already
 // downloaded earlier but the user did not tap "Update Now" yet.
 ...
 async function applyUpdate() {
  // Updating clears the saved copy, so it must never run without a working internet connection
  let reachable = navigator.onLine;
  if (reachable) {
   try { await fetch('index.html', { cache: 'no-store' }); } catch (e) { reachable = false; }
  }
  if (!reachable) {
   showToast("Connect to the internet to update / અપડેટ માટે ઇન્ટરનેટ ચાલુ કરો", 'warning');
   return;
  }
  ...
  // keep language, theme and the "already downloaded all entries" markers
  const keep = {};
 ...
 function checkUpdateManual() {
  toggleMenu();
  if (!navigator.onLine) {
   showToast("No internet connection / ઇન્ટરનેટ નથી", 'warning');
   return;
  }
...
if (bannerSpan) {
 ...
 // Returns the saved stone that already uses this packet number (or null).
 // Works offline too, because every entry is kept on the phone.
 let editOriginalBarcode = '';
 async function findDuplicatePacket(barcode, excludeId) {
  if (!barcode || barcode === "No ID" || !auth.currentUser) return null;
  try {
   const snap = await fsGet(db.collection("users").doc(auth.currentUser.uid).collection("stones").where("barcode", "==", barcode));
   let found = null;
   snap.forEach(d => { if (!found && d.id !== excludeId) found = d.data(); });
   return found;
  } catch (e) {
 ...
 function duplicateMsg(d) {
  return "Packet #" + d.barcode + " already saved (" + d.date + ", " + d.weight + "ct) / આ પેકેટ નંબર પહેલેથી સેવ છે";
 }
 async function warnIfDuplicatePacket() {
  const dup = await findDuplicatePacket(document.getElementById('barcode').value.trim(), null);
  if (dup) showToast(duplicateMsg(dup), 'warning', 4000);
 }
 (function () {
  const bc = document.getElementById('barcode');
  if (bc) bc.addEventListener('change', warnIfDuplicatePacket);
 })();
...
async function saveStone() {
  ...
  try {
    ...
    const dup = await findDuplicatePacket(s.barcode, null);
    if (dup) { showToast(duplicateMsg(dup), 'warning', 4000); return; }
 ...
 async function updateStone() { 
  ...
  if (data.barcode !== editOriginalBarcode) { // only check when the packet number was changed
   const dup = await findDuplicatePacket(data.barcode, id);
   if (dup) return showToast(duplicateMsg(dup), 'warning', 4000);
  }
